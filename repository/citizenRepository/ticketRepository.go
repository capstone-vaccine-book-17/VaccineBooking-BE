...
func (u *citizenRepository) GetAllTicket(citizenId uint) ([]citizenDto.TicketDetails, error) {
	...
	// dik := "proses"
	if err := u.db.Model(&model.Booking{}).Select("bookings.*, citizens.name, citizens.nik, citizens.gender, vaccine_varieties.name as vaccine, medical_facilitys.name as rs_name,sessions.dosis,sessions.date,DATE_FORMAT(sessions.date, '%d %M %Y') as conv_date,sessions.start_time,sessions.end_time").
